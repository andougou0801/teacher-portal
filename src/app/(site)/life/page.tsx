...
// 国家公務員の休暇制度（人事院規則15-14）を「一例」として紹介する一覧。
// 教員を含む地方公務員は都道府県・市区町村ごとに条例が異なり、種類の数・分類・
// 日数のいずれも、この一例とは違う場合がある（実際、岐阜市の規則を調べたところ
// 項目数・分類・日数のいずれにも国基準との相違が見つかった）。あくまで
// 「こういう種類の休暇がある」というイメージをつかむための参考として掲載している。
...
const tokubetsuKyukaItems: SubItem[] = [
  ...
  },
  {
    name: "夏季休暇",
    article: "第22条1項15号",
    duration: "国の基準は7〜9月の間で連続3日以内",
    note: "これは国家公務員の基準日数です。地方公務員（教員含む）はこれより多いことが一般的で、自治体によっては5〜6日程度の場合もあります。実際の日数は勤務先の条例でご確認ください。",
    common: true,
  },
  {
    name: "現住居の滅失・破損（災害等）",
...
const termGroups: TermGroup[] = [
  {
    ...
    groupNote:
      "以下は国家公務員の基準（人事院規則15-14）を「一例」として紹介したものです。教員も含めた地方公務員は都道府県・市区町村ごとに条例が定められており、休暇の種類の数・分類・日数のいずれも、この一例とは異なる場合があります。まずはこのページで大まかなイメージをつかんだうえで、正確なところは必ずお勤め先の都道府県・市区町村の公式サイトでご確認ください。",
    terms: [
      {
        name: "年次有給休暇（年休）",
        description:
      ...
      },
      {
        name: "特別休暇",
        description:
          "結婚・忌引・出産・介護・災害など、特別な事情があるときに認められる休暇です。国の基準（人事院規則15-14 第22条）では下記のように号（条文の中の項目番号）ごとに細かく種類が分かれています。号をタップすると詳細が開きます。ただし種類の数・分類・日数は自治体ごとに違うことが多いので、あくまで「一例」としてご覧ください。",
  ...
  return (
    <section className="mx-auto max-w-3xl px-8 py-14">
      ...
      <div className="mb-6 rounded-2xl border border-warn-line bg-warn-bg p-4 text-sm text-warn">
        ここで紹介している制度・日数は、国家公務員の基準を「一例」として使っています。
        休暇の種類の数・分類・日数は都道府県・市区町村ごとに条例で定められており、
        この一例とは異なることが珍しくありません。まずは大まかなイメージをつかむための
        参考として使い、実際に使うときは必ずお勤め先の都道府県・市区町村の公式サイトや
        教育委員会・共済組合でご確認ください。
      </div>
      ...
      <div className="mt-10 rounded-2xl border border-line bg-white p-5">
        ...
        <p className="mt-4 text-sm text-muted">
          ...
          「教職員 互助会」で検索すると、同じような制度の窓口が見つかります。休暇の正式な
          種類・日数を条例で確認したいときは、都道府県・市区町村名＋「職員 勤務時間 休暇 条例」
          で検索すると、公式の条例・規則のページが見つかります。
        </p>
      </div>
    </section>
  );

...
- 自作ツールカタログ（`/tools`）：先生向け実働ツール15本（週の時間割メーカー／算数プリントメーカー／漢字テスト作成／座席表作成／班分けツール／リーグ戦・トーナメント表メーカー／名簿関連ツール／点数・平均計算／授業用タイマー／抽選・あみだくじツール／文章問題メーカー／100マス計算プリントメーカー／通知表所見文例集／議事録メーカー／国語の文章作成メーカー）。「作成予定」ツールは現在なし
- 子ども向けツール一覧（`/kids`）：実働ツール6本（筆算マスター・都道府県マスターゲーム・漢字れんしゅう・歴史人物タイムトラベルクイズ・タイピングれんしゅう・角度マスター）
...
6. （導入済みの場合は追加で）通報機能を使うには、SQL Editorで
   [`supabase/002_reports.sql`](./supabase/002_reports.sql) の中身も貼り付けて実行してください。
   未実行でも `/community` 自体は問題なく動作します（通報件数が常に0として扱われるだけです）。

設定前は `/community` に「準備中」の案内が表示されるだけで、ビルドやその他のページには影響しません。
荒らし・不適切投稿への対応は、①同じ投稿への通報が3件たまると自動的に本文が「確認中」表示に切り替わる
仕組み（`reports`テーブル）と、②実際の削除はこれまで通りSupabaseの「Table Editor」から
questions / answers テーブルの該当行を手動で削除する運用、の2段構えです。
